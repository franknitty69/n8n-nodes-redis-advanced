  ...
  description: INodeTypeDescription = {
    ...
    properties: [
        ...
        options: [
          {
            ...
            value: "automatic",
            description:
              "Requests the type before requesting the data (slower)",
          },
          {
            name: "Hash",
            value: "hash",
      ...
      },
      {
        ...
        required: true,
        description:
          "Score-member pairs in format: score1 member1 score2 member2",
      ...
      for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
        try {
          ...
          if (operation === "delete") {
            ...
            const keyType = this.getNodeParameter(
              "keyType",
              itemIndex
            ) as string;
            ...
            const expire = this.getNodeParameter(
              "expire",
              itemIndex,
              false
            ) as boolean;
            const ttl = this.getNodeParameter("ttl", itemIndex, -1) as number;
            const setMode = this.getNodeParameter(
              "setMode",
              itemIndex,
              "always"
            ) as "always" | "nx" | "xx";
            ...
            const redisList = this.getNodeParameter(
              "list",
              itemIndex
            ) as string;
            const messageData = this.getNodeParameter(
            ...
            const tail = this.getNodeParameter(
              "tail",
              itemIndex,
              false
            ) as boolean;
            ...
            const keyValuePairs = this.getNodeParameter(
              "keyValuePairs",
              itemIndex
            ) as string;
            ...
            const pattern = this.getNodeParameter(
              "pattern",
              itemIndex
            ) as string;
            ...
            const members = this.getNodeParameter(
              "members",
              itemIndex
            ) as string;
            ...
            const sortedSet = this.getNodeParameter(
              "sortedSet",
              itemIndex
            ) as string;
            const scoreMembers = this.getNodeParameter(
              "scoreMembers",
              itemIndex
            ) as string;
            ...
            const withScores = this.getNodeParameter(
              "withScores",
              itemIndex
            ) as boolean;
            let result;
            ...
            const keyArray = keys
              ? keys.split(/\s+/).filter((k) => k.length > 0)
              : [];
            const argArray = args
              ? args.split(/\s+/).filter((a) => a.length > 0)
              : [];
